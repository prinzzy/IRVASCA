...
class AdminEmailController extends Controller
{
    ...
    {
        $emailsPerMonth = Cache::remember('emails_per_month', 3600, function () {
            return Email::select(DB::raw('YEAR(created_at) as year'), DB::raw('MONTH(created_at) as month'), DB::raw('COUNT(*) as count'))
                ->groupBy('year', 'month')
                ->orderBy('year', 'asc')
                ->orderBy('month', 'asc')
                ->get()
    ...
    {
        $emailCountsByDay = Cache::remember('email_counts_by_day', 3600, function () {
            return Email::select(DB::raw('DATE(created_at) as date'), DB::raw('count(*) as count'))
